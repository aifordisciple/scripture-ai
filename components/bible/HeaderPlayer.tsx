...
export function HeaderPlayer({ player, text, className, mode = 'full' }: HeaderPlayerProps) {
  ...
  // --- 模式 2: 完整模式 (带进度条和倍速) ---
  return (
    <div className={cn("flex items-center gap-2 bg-secondary dark:bg-card rounded-full px-3 py-1 border border-border dark:border-border transition-all", className)}>
      ...
      >
        {isLoading ? (
          ...
          <Play className="h-4 w-4 text-foreground dark:text-foreground/80 fill-current ml-0.5" />
        )}
      </Button>
      ...
      <div className={cn("flex items-center gap-2 transition-all duration-500 overflow-hidden", 
        ...
        <span className="text-[10px] text-muted-foreground font-mono w-9 text-right shrink-0 select-none">
        ...
        <span className="text-[10px] text-muted-foreground/60 font-mono w-9 shrink-0 select-none">
          {formatTime(duration)}
        </span>
      </div>
