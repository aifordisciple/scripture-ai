...
export function SermonListPanel() {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-[#1d1d1f]">
      {/* Search — Apple pill search input */}
      <div className="px-4 pt-4 pb-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm rounded-full border border-black/[0.08] dark:border-white/[0.08] bg-[#f5f5f7] dark:bg-white/[0.06] placeholder:text-[#7a7a7a] focus:outline-none focus:border-[#0066cc]/40 focus:ring-2 focus:ring-[#0066cc]/20 transition-shadow"
          />
        </div>
      </div>

      {/* Folder section — Apple nav-link density */}
      <div className="px-3 pb-2 border-b border-[#f0f0f0] dark:border-white/[0.06]">
        <div className="flex items-center justify-between px-2 mb-1">
          ...
          <button
            ...
            className="text-[#7a7a7a] dark:text-[#999] hover:text-[#1d1d1f] dark:hover:text-white transition-colors"
        ...
        <button
          ...
          className={cn(
            'w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs transition-all duration-150',
            !sermonSelectedFolderId
          ...
          <div key={folder.id}>
            ...
            {expandedFolders.has(folder.id) && folder.children.map(child => (
              <button
                ...
                className={cn(
                  'w-full flex items-center gap-2 pl-8 pr-2.5 py-1.5 rounded-lg text-xs transition-all duration-150',
      ...
      {/* Tags — Apple configurator-option-chip style */}
      {allTags.length > 0 && (
      ...
      {/* Sermon list — Apple store-utility-card items */}
          ...
          filteredSermons.map(sermon => {
            ...
            return (
              <div
                ...
                className={cn(
                  'group cursor-pointer rounded-[18px] p-4 transition-all duration-200',
                  selected
              ...
              >
                {/* Title — Apple body-strong (17px/600) scaled to list density */}
                <h3 className={cn(
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <button
                    ...
                    className={cn(
                      'p-1 rounded-lg transition-all duration-150 opacity-0 group-hover:opacity-100 active:scale-95',
                      selected
                        ? 'text-white/60 hover:text-white hover:bg-white/10'
                        : 'text-[#7a7a7a] hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30'
                    )}
      ...
      {/* New sermon — Apple pill CTA */}
      <div className="px-4 py-3 border-t border-[#f0f0f0] dark:border-white/[0.06]">
        <button
          onClick={() => setShowNewDialog(true)}
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-full bg-[#0066cc] hover:bg-[#0071e3] text-white text-sm font-normal transition-colors active:scale-95"
        >
      ...
      {/* Context menu — Apple frosted glass */}
      {contextMenu && (
        <div
        ...
        >
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 text-xs text-red-500 hover:bg-black/[0.04] dark:hover:bg-white/[0.06] transition-colors"
          >
            <Trash2 className="w-3 h-3" />
            {t('sermon.delete')}
          </button>
