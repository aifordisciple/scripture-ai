...
每次处理新 bug 时，必须先阅读 `docs/experience.md`，检查是否有可借鉴的经验教训，避免重复踩坑。

## 修复后：总结经验写入文档

每次 bug 修复完成后，必须将修复过程中的经验总结追加到 `docs/experience.md` 文档中。总结应包含：

1. **场景描述**：什么功能、什么场景下出现的 bug
2. **现象**：bug 的具体表现
3. **根因分析**：为什么会出问题，而不仅仅是"改了什么"
4. **经验法则**：提炼出可复用的规则，避免同类问题再次发生
5. **调试策略**（如适用）：有效的调试方法，踩过的坑
